...
Everything that has to agree with `lazy accept` about a parent branch — sync's merge target, the base a new child branch is cut from, the base a turn diff is rendered against — resolves it through `resolveUpstreamMergeRef()` (`src/remote/upstream-ref.ts`), never through `driver.resolveUpstreamRef()` directly. Accept routes an UNPROTECTED target (every intermediate `lazy/...` branch, plus any unprotected named branch) into the LOCAL branch and only a protected target through the forge; `mergeLandsLocally()` is that single routing predicate, and both accept and the resolver call it. Resolving unconditionally to `origin/<parent>` is what produced the fix-sync-stale-origin-parent incident: a parent task's own agent commits can never be on origin (agents have no push credentials), so sync answered "Already up to date" while accept refused with conflicts, and its advice to run `lazy sync` was unactionable.

The opposite miss must not regress (fix-upstream-ref / fix-upstream-remote): the local parent can be BEHIND origin after a forge-side accept, so the remote ref still wins whenever it CONTAINS the local branch. The two only disagree when local has commits origin lacks — and there accept's routing decides. When they disagree, the resolution WARNS (naming both refs, which was used, and whether the branch is pushable) and **never pushes the parent**: the engineer may be mid-manual-work on it.
...
A task worktree's Dockerfile.lazy must never silently pick the container image: task branches are agent-writable, and deriving the image from one would let an agent's Dockerfile edits run as build steps under the daemon's docker on the HOST, outside every container guard. Launch paths (`launchSupervisorAsync`, `runClaude`, `ensureImage`) therefore never anchor image resolution at `sandbox.worktreePath`.

**Human-consented exception (CLI TTY only) — per-task pin:** on `lazy create` / `lazy start` / `lazy edit`, when stdin is a TTY and cwd is inside a task worktree whose `Dockerfile.lazy` content hash differs from the reference (`origin/<default-branch>` when present, else the project root's resolved `[docker].dockerfile`), lazy asks once (default NO) whether to build that Dockerfile and pin it on the task. On yes: build content-addressed `lazy-custom-<hash>`, persist `metadata.custom_image` + `metadata.custom_image_hash` + `metadata.custom_image_context` (worktree HEAD when built — provenance only), and every later turn (unblock/sync/ask/pre-accept) uses that stored image with no prompt. Subtasks inherit the parent's pin on create; `lazy clone` / `lazy redo` (and their MCP counterparts) are fresh starts that use the root image and warn when the source or previous attempt had a pin. If the stored image is missing at launch, fail loudly — never fall back to the root image. Non-TTY, `--yes`, MCP, builder, and scripts are never prompted and always use the root-resolved image (or a daemon adoption below); there is deliberately no MCP surface for choosing images.

**Human-consented exception (CLI TTY only) — upgrade adoption:** on `lazy upgrade` (incl. `--images`) from a TTY inside a task worktree whose Dockerfile differs, lazy asks once (default NO) whether to adopt that Dockerfile for the image build AND the restarted daemon. On yes: build content-addressed `lazy-custom-<hash>` and persist adoption in daemon runtime state (`~/.lazy/daemon/<slug>/adopted-image.json` — not an env var, not lazy.toml), recording the worktree HEAD as provenance alongside the hash. Launches soft-pin the stored image name (never re-hash the worktree path — a post-consent agent edit must not become a host docker rebuild); if the image is missing, fail loud. If the Dockerfile content drifts from the consented hash, or `lazyVersion` mismatches, or the path is gone, adoption is cleared. Sticks until the next upgrade rebuild decides again. Visible: daemon startup log, `lazy doctor`, `lazy upgrade` output.

**Build context follows the consented Dockerfile** (engineer-approved invariant amendment, 2026-09-06, after a build in the wild used a worktree's Dockerfile with the main checkout as context and its `COPY` of a branch-only file failed): a human-consented worktree Dockerfile builds with **that worktree directory** as the docker build context — never the project root, which is the bug this rule exists to prevent — while the Dockerfile bytes themselves stay the consented temp copy; the plain root `[docker].dockerfile` path keeps the project root as its context. The image name is `lazy-custom-<hash of Dockerfile content AND context directory>` so two worktrees holding the same `Dockerfile.lazy` get two images (the bare content hash is still what `custom_image_hash` stores, because drift detection compares Dockerfile bytes). Full rationale: [docs/design/worktree-image-flow.md](docs/design/worktree-image-flow.md).

**Launch resolution order:** (1) task's stored `custom_image` — fail loud if missing; (2) valid daemon-adopted image; (3) root `[docker].dockerfile`.

**Root resolution** (`resolveCustomDockerfile` in `src/capture/claude.ts`) joins `[docker].dockerfile` to the PROJECT ROOT when nothing above applies. `lazy upgrade` (incl. `--images`) calls `enableUpgradeImageBuild()` at command entry so the rebuild may path-resolve a just-written adoption; every other process soft-pins `adopted.imageName` instead and never re-hashes the worktree file. There is no env-var override — the interactive TTY flows above replaced it.
