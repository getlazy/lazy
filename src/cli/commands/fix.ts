...
export async function commandFix(args: string[]): Promise<void> {
  ...
  // Before the bug report and its prompt is typed: the daemon refuses a write it cannot
  ...
  try {
    ...
    const agentResolution = resolveAgentForNewTaskFromConfig(
      { inheritFrom: parent, taskType: 'fix' },
      config.agent,
      projectSettings,
    );
    const t = await storage.createTask(goal, parentTaskId, undefined, code, 'fix', agentResolution.agentId);
