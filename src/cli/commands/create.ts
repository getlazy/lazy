...
export async function commandCreate(args: string[]): Promise<void> {
  ...
  try {
    ...
    const t = await storage.createTask(goal, parentTaskId, undefined, code, taskType ?? undefined);
...
export function createUsage(): void {
  console.log(`Usage: lazy create [--goal <goal>] [--prompt <text>] [--model <model>] [--type <type>] [--code <code>] [--parent <task_id>]
