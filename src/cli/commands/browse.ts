/**
 * `lazy browse <task_id>` — TUI-based read-only artifact browser.
 *
 ...
 *
 * This is the former `lazy review` TUI. `lazy review` is now the agent-review
 * verb.
 */
...
export async function commandBrowse(args: string[]): Promise<void> {
  ...
  if (!process.stdin.isTTY) {
    console.error('lazy browse requires an interactive terminal.');
    process.exit(1);
  }
  ...
  try {
    ...
    // Default full-screen browser — pre-flight: runner availability.
...
export function browseUsage(): void {
  console.log(`Usage: lazy browse <task_id> [-i]

Browse a task's artifacts. Default mode opens a full-screen two-panel
browser of the task's artifacts (response, plan, diff, comments, commits).
...
Actions (use CLI after exiting browse):
  ...
  lazy review <task_id>                  Run an agent review of the work

Examples:
  lazy browse abc123          Full-screen browser
  lazy browse -i abc123       Hunk-by-hunk interactive review`);
}
