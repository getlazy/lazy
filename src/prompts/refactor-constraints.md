...
- **If you discover bugs** while refactoring, propose them as separate tasks via `lazy_propose`. Do not fix bugs in a refactoring task — that mixes concerns and makes review harder.
- **If you discover missing tests** that verify existing behavior, add them first — they serve as your safety net for the refactoring. Propose tests for *new* functionality as separate tasks.
