 ...
 * `lazy pair` (task mode and branchless), `lazy chat`, and Pair/Chat over the
 * web shell.
 *
 ...
 *
 * This is the SINGLE seam every call site uses, so role resolution, reachability
 * preflight, surface conversion and credential sourcing cannot drift between
 * them. It lives beside the other credential resolution rather than under
 * `src/cli/`, because the callers are not all CLI: the interactive supervisor
 * relaunches through it, and the daemon's web shell builds the Pair/Chat exec
 * env with it.
 *
...
export class InteractiveCredentialError extends Error {
  constructor(command: string, daemonDetail: string, shellVar: string | null) {
    super(
      ...
      `  - Store a credential so no shell has to carry it, then restart:\n` +
      `      claude setup-token | lazy auth set anthropic && lazy daemon restart\n` +
      `  - Or restart it from a shell that HAS the credential exported:  lazy daemon restart\n` +
 ...
 *
 * The surface DEFAULTS to `'host'` — `lazy chat` and branchless `lazy pair` run
 * Claude Code as a host process even on a docker-runner project, so every
 * address handed to them must be host-reachable, the same conversion
 * `preflightRoleTarget` probed. Task-mode `lazy pair` runs the session in the
 * task's container and passes `'container'` instead. See {@link LaunchSurface}.
 *
...
export async function resolveInteractiveLaunch(
  ...
  opts?: {
    /**
     * Where the resolved env will be USED. Defaults to `'host'`.
     *
     * `lazy pair <task>` runs the session inside the task's container and passes
     * `'container'`: the addresses it needs are the ones a container can reach
     * (host.docker.internal, not localhost), exactly as a supervised turn's are.
     */
    surface?: LaunchSurface;
    /**
     * JIT-credential identity. Defaults to this project's shared host-builder
     * grant. An in-container pair session passes a TASK-scoped identity so the
     * audit log attributes its model calls to that task, not to "some host
     * builder session on this project".
     */
  ...
  const surface: LaunchSurface = opts?.surface ?? 'host';
  ...
  const envVars = await resolveAuthEnvFromDaemon(
    ...
    surface,
    config,
    // The profile is the proxy's routing key, so the default identity carries
    // the one this session actually resolved — a pinned builder must not be
    // preflighted against its own endpoint and then routed to the primary.
    opts?.identity ?? {
      role: 'builder', taskId: null, label: `host-builder:${root}`, profile: proxied.profile,
    },
  );

  // A profile whose endpoint authenticates nobody (a local model server) is
  // launched with a synthetic token, so there is no daemon credential to check
  // for and nothing to fail on.
  if (!usesSyntheticCreds(proxied) && !hasCredential(envVars)) {
