...
- **`lazy timings` shows where a slow command actually spent its time** — every request is traced automatically (always on, nothing to configure) and rendered as a duration tree that flags the slowest step, stitched across the CLI→daemon hop; `lazy start` is instrumented first. Measured cost is 0.7µs per span and the trace store is pruned to a bounded size. See `docs/spikes/timings.md` (`spike-lazy-timings`)
...
- **A broken lazy.toml fails loudly instead of silently running on defaults**, naming the offending line; an overrunning dashboard page now returns a readable error, not a dead connection (`harden-daemon-routes`)
