 ...
 *
 * WHY THIS EXISTS: tests that need a daemon on its DEFAULT PID/token/marker
 * paths used to isolate themselves from
 ...
 * test that only wanted private daemon state silently changes half a dozen other
 ...
 * `src/daemon/paths.ts`: every daemon path (PID, token, log, lock, web
 ...
 * which keeps the resulting paths short. That is hygiene, not a fix:
