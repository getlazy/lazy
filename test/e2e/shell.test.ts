...
describe('lazy shell', () => {
  ...
  test('runs a command in the worktree and exits with its exit code', async () => {
    ...
    const result = await ctx.lazy(['shell', taskId, '--host', '--', 'touch', 'RAN_HERE.txt']);
  ...
  test('sets LAZY_TASK and runs with cwd at the worktree', async () => {
    ...
    const result = await ctx.lazy(['shell', taskId, '--host', '--', 'sh', '-c', 'echo "$LAZY_TASK at $(pwd)"']);
  ...
  test('propagates a non-zero exit code from a failing command', async () => {
    ...
    const result = await ctx.lazy(['shell', taskId, '--host', '--', 'sh', '-c', 'exit 7']);
  ...
  test('errors when no command is given after --', async () => {
    ...
    const result = await ctx.lazy(['shell', taskId, '--host', '--']);
  ...
  test('errors when the task has no session before running a -- command', async () => {
    ...
    const result = await ctx.lazy(['shell', taskId, '--host', '--', 'touch', 'SHOULD_NOT_EXIST.txt']);
  ...
  test('errors when the task cannot be resolved', async () => {
    const result = await ctx.lazy(['shell', 'nonexistent', '--host', '--', 'git', 'status']);
